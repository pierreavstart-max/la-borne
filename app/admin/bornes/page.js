...
import { getBornes, addBorne, deleteBorne } from '../../lib/db';
...
export default function BornesPage() {
  ...
  async function handleAdd(e) {
    e.preventDefault();
    setSaving(true);
    await addBorne({ nom, ref: genRef(nom), adresse, client, orient, statut });
    setNom(''); setAdresse(''); setClient('');
                ...
                >
                  ...
                  <td style={{padding:'11px 12px'}}>
                    <span style={{fontSize:'10px',fontWeight:'600',padding:'2px 8px',borderRadius:'20px',
                      background:b.orient==='Portrait'?'#F0ECFB':'#F7F6F3',
                      color:b.orient==='Portrait'?'#5B3DB8':'#6B6860'
                    }}>{b.orient}</span>
                  </td>
                  <td style={{padding:'11px 12px'}}>
                    <span style={{fontSize:'10px',fontWeight:'600',padding:'2px 8px',borderRadius:'20px',
                      background:b.statut==='En ligne'?'#E6F5ED':'#FCEAEA',
                      color:b.statut==='En ligne'?'#18865A':'#C02B2B'
                    }}>{b.statut}</span>
                  </td>
                  <td style={{padding:'11px 12px'}}>
