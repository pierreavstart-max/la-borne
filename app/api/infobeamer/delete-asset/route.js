...
export default function BornesClientPage() {
  ...
  async function handleDelete(demande) {
    ...
    try {
      // Supprime sur info-beamer si l'asset existe
      ...
      // Archive la demande dans Firestore
  ...
  const borneComms = demandes.filter(d => (d.statut === 'Approuvée' || d.statut === 'Diffusée') && !d.archived);
